...
from sm.outputs.semantic_model import SemanticModel, SemanticType
...
class SemanticTyper(object):
    ...
    def __init__(self,
                 dataset: str,
                 train_examples: list[Example[FullTable]],
                 exec_dir: Path) -> None:
        self.dataset = dataset
        self.train_source_ids = {ex.table.table.table_id for ex in train_examples}
        self.exec_dir = exec_dir
        self.exec_dir.mkdir(exist_ok=True, parents=True)

        self.model = None
        self.stype_db = SemanticTypeDB.get_stype_db(dataset, [ex.table.table.table_id for ex in train_examples], self.exec_dir)
    ...
    def semantic_labeling_v2(self, sms: List[SemanticModel], top_n: int) -> None:
        ...
        if self.model is None:
            ...
            if model_file.exists():
                logger.debug("Load previous trained model...")
                self.model = serde.pickle.deser(model_file)
            else:
                logger.debug("Train new model...")
                ...
                serde.pickle.ser(clf, model_file)
                self.model = clf
